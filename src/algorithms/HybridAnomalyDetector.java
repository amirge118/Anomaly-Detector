...
import model.Helpclass.CorrelatedFeatures.CircleCorrelatedFeatures;
import model.Helpclass.*;
...
public class HybridAnomalyDetector implements TimeSeriesAnomalyDetector {
    private HashMap<String, AlgorithmMinCircle.LineCorrelatedFeatures> LinecorrelatedFeaturesList;
    ...
    public void learnNormal(TimeSeries ts) {
        ...
        for (int i = 0; i < ts.getHashMap().size() ; i++) { //for every feature checking cov with the other features
            ...
            if (best_correlated >= 0.95) {
                ...
                max_dev *= (float) 1.1;
                this.LinecorrelatedFeaturesList.put(feature_check, new AlgorithmMinCircle.LineCorrelatedFeatures(feature_check, through_feature, best_correlated, reg_line, max_dev));
    ...
    public HashMap<String,List<AnomalyReport>> detect(TimeSeries ts) {
        float[] curColToCheck;
        Float[] curArray;
        for (AlgorithmMinCircle.LineCorrelatedFeatures correlatedFeatures : this.LinecorrelatedFeaturesList.values()) {
    ...
    public float[] detect_P(TimeSeries ts, String feature, int time_step) {
        ...
        if (zscoremap.containsKey(feature)) {
            ...
            AlgorithmMinCircle.LineCorrelatedFeatures correlatedFeature = LinecorrelatedFeaturesList.get(feature);

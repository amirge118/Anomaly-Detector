...
public class SimpleAnomalyDetector implements TimeSeriesAnomalyDetector {
	...
	public void learnNormal(TimeSeries ts) {
		...
		for (int i=0;i<ts.getHashMap().size()-1;i++) { //for every feature checking cov with the other features
			...
			max_dev*=(float)1.1;
			this.correlatedFeaturesList.put(feature_check,new AlgorithmMinCircle.LineCorrelatedFeatures(feature_check, through_feature, best_correlated, reg_line, max_dev));
		}
	}
	@Override
...
//		return points;
	...
	public float[] detect_P(TimeSeries ts,String feature,int time_step){
		AlgorithmMinCircle.LineCorrelatedFeatures correlatedFeature= correlatedFeaturesList.get(feature);
